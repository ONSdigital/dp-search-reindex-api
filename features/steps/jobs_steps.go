...
func (f *JobsFeature) theResponseShouldContainValuesThatHaveTheseStructures(table *godog.Table) error {
	...
	//If indexName is currently "" then the response needs to be read
	if indexName == "" {
	...
	err = f.checkStructure(id, lastUpdated, expectedResult, links, indexName)
	if err != nil {
		return fmt.Errorf("failed to check that the response has the expected structure: %w", err)
	}
...
func (f *JobsFeature) readResponse(err error, response models.Job) error {
	f.responseBody, _ = io.ReadAll(f.APIFeature.HttpResponse.Body)
	err = json.Unmarshal(f.responseBody, &response)
	if err != nil {
		return fmt.Errorf("failed to unmarshal json response: %w", err)
	...
	lastUpdated = response.LastUpdated
	links = response.Links
	indexName = response.SearchIndexName
	return err
}
...
func (f *JobsFeature) theReindexrequestedEventShouldContainTheExpectedJobIdAndSearchIndexName() error {
	assert.Equal(&f.ErrorFeature, id, f.reindexRequestedData.JobID)
	assert.Equal(&f.ErrorFeature, indexName, f.reindexRequestedData.SearchIndex)
	return f.ErrorFeature.StepError()
}
